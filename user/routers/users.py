...
@router.post("/{user_id}")
async def update_user(request: _fastapi.Request,
                      user_id: int,
                      current_user: _schemas.TokenData = _fastapi.Depends(_oauth2.get_current_user),
                      db: AsyncSession = _fastapi.Depends(_services.get_db),):
    ...
    if method and method == "PUT":
        ...
        if user is None:
            raise _fastapi.HTTPException(status_code=404, detail="User does not exist")
        update_form = _schemas.UpdateForm(nickname=form_data.get("nickname"),
                                          old_password=form_data.get("old_password"),
                                          new_password=form_data.get("new_password"))
        await _services.update_user(update_form=update_form, user=user, db=db)
        return "Successfully updated the user"
    else:
        # reserve for create user through html form
        raise _fastapi.HTTPException(status_code=404, detail="Page does not exist")
